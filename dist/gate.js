...
async function judge(intent, agent) {
    const system = `You decide whether a specialized agent can fulfill a user's task. ` +
    ...
    try {
        return await llmJson(system, user);
    }
    catch {
...
export async function pickBestAgent(intent, candidates, trace) {
    if (candidates.length === 0)
        return null;
    const judged = await Promise.all(candidates.slice(0, TOP_N).map(async (agent) => ({
        agent,
        judgement: await judge(intent, agent),
    })));
    // Log every judgement — this is the demo-legible part.
    for (const j of judged) {
        trace.push(`gate: "${j.agent.name}" capable=${j.judgement.capable} ` +
            `conf=${j.judgement.confidence.toFixed(2)} — ${j.judgement.reason}`);
    }
    const best = judged
        .filter((j) => j.judgement.capable && j.judgement.confidence >= CONFIDENCE_THRESHOLD)
        .sort((a, b) => b.judgement.confidence - a.judgement.confidence)[0];
    if (!best) {
        trace.push(`gate: nothing cleared threshold ${CONFIDENCE_THRESHOLD} — falling through`);
        return null;
    }
