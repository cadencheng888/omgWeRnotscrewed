...
export async function searchAgents(searchText, trace, opts = {}) {
    ...
    try {
        ...
        trace.push(`agentverse: search failed (${e.message})`);
        return [];
    }
    ...
    if (process.env.DEBUG_AGENTVERSE) {
        for (const a of results) {
            console.log(`    [debug] ${a.name} [${a.status}, recent=${a.recent_interactions}]`);
        }
    }
    ...
    const filtered = results.filter((a) => {
        ...
        if (activeOnly && a.status && a.status !== "active")
            return false;
