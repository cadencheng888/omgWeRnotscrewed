...
function openInBrowser(url) {
    ...
    exec(cmd, (err) => {
        if (err)
            console.error("Could not auto-open live view:", err.message);
    });
}
...
async function openLiveView(sessionId, trace) {
    if (!sessionId) {
    ...
    try {
        ...
        trace.push(`browserbase: couldn't open live view (${e.message})`);
    }
}
...
export async function runBrowserbase(intent, trace) {
    trace.push(`browserbase: handling "${intent}"`);
    if (!ANTHROPIC_KEY) {
        trace.push("browserbase: ANTHROPIC_API_KEY not set — required in direct/disableAPI mode");
    ...
    try {
        ...
        const run = await agent.execute({
            instruction: `Accomplish this task on the web: ${intent}.\n` +
            ...
            output: z.object({
                ...
                actionTaken: z
                    .string()
                    .describe("the concrete action performed, e.g. 'added MacBook Air to cart', or 'none' if read-only"),
                stoppedBecause: z
                    .string()
                    .describe("why the agent stopped if it didn't fully complete, e.g. 'reached login wall', or 'completed' otherwise"),
                success: z
                    .boolean()
        ...
        trace.push(`browserbase: failed (${e.message})`);
        return { source: "browserbase", status: "failed", payload: null, trace };
    }
    finally {
